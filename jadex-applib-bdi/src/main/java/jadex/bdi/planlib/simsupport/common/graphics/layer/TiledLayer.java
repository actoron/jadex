...
public class TiledLayer implements ILayer
{
    ...
    public void draw(IVector2 areaSize, ViewportJ2D vp, Graphics2D g)
    {
        ...
        for (double x = 0.0; x < areaSize.getXAsDouble(); x = x + tileSize_.getXAsDouble())
        {
        	for (double y = 0.0; y < areaSize.getYAsDouble(); y = y + tileSize_.getYAsDouble())
        	{
    ...
    public synchronized void draw(IVector2 areaSize, ViewportJOGL vp, GL gl)
    {
        ...
        for (float x = 0.0f; x < areaSize.getXAsFloat(); x = x + tileSize_.getXAsFloat())
        {
        	for (float y = 0.0f; y < areaSize.getYAsFloat(); y = y + tileSize_.getYAsFloat())
        	{
        		...
        		gl.glVertex2f(x + tileSize_.getXAsFloat(), y + tileSize_.getYAsFloat());
    ...
    public boolean equals(Object obj)
    {
    	if (obj instanceof TiledLayer)
    	{
    		...
    		return ((tileSize_.equals(other.tileSize_)) &&
    				(texturePath_.equals(other.texturePath_)));
    	}
    	return false;
    }
}
