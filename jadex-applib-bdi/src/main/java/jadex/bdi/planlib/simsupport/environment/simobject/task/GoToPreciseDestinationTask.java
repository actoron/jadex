...
/** Go precisely to the destination specified, even using sub-stimulation time steps
 *  if necessary.
 */
public class GoToPreciseDestinationTask implements ISimObjectTask
{
	...
	public GoToPreciseDestinationTask(IVector2 targetPosition,
			   				   		  IVector1 speed)
	{
		this(DEFAULT_NAME, targetPosition, speed);
	}
	
	public GoToPreciseDestinationTask(String name,
	...
	public void shutdown(SimObject object)
	{
		synchronized (object)
		{
			...
			SimulationEvent evt = new SimulationEvent(SimulationEvent.GO_TO_PRECISE_DESTINATION_REACHED);
	...
	public void execute(IVector1 deltaT, SimObject object)
	{
		synchronized (object)
		{
			...
			// go back to "b", have the same wait and we are in "a" 
			if (currentPosition.getDistance(targetPosition_).less(moveTask_.getLastMovementDelta().getLength()))
			{
				// Destination reached, stop, set correct position and trigger event.
