...
public class GoToDestinationPlan extends Plan
{
	...
	public void body()
	{
		...
		removeTask.getParameter("task_name").setValue(
				GoToDestinationTask.DEFAULT_NAME);
		dispatchSubgoalAndWait(removeTask);

		ISimObjectTask task = new GoToDestinationTask(destination, speed,
				tolerance);
		...
		while(!evt.getParameter("type").getValue().equals(
