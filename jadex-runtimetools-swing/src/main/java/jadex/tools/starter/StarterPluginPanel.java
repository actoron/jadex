...
//										IResourceIdentifier rid = createResourceIdentifier();
										SComponentFactory.isStartable(jcc.getPlatformAccess(), filename, rid).addResultListener(new SwingDefaultResultListener(spanel)
										{
											...
											{
												if(((Boolean)result).booleanValue())
													StarterPanel.createComponent(jcc, rid, filename, null, null, null, false, null, null, null, null, null, null, null, StarterPluginPanel.this);
												mpanel.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
											}
										});
									}
		...
		comptree = new ComponentTreePanel(jcc.getPlatformAccess(), jcc.getCMSHandler(), jcc.getIconCache(), JSplitPane.HORIZONTAL_SPLIT);
