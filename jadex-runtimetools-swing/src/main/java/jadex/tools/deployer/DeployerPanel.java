...
//															second.refreshTreePaths(null);
	 ...
	 * 
	 */
	public static void copy(final String sel_1, final String sel_2, 
		final DeployerServiceSelectorPanel first, final DeployerServiceSelectorPanel second) 
	{
		if(sel_1!=null && sel_2!=null)
		{
			final IComponentIdentifier lcid = first.getJCCAccess().getComponentIdentifier();
			final IDeploymentService ds = second.getDeploymentService();
...
//												second.refreshTreePaths(null);
									...
									catch(final Exception ex)
									{
										...
										jccacc.scheduleStep(new IComponentStep<Void>()
										{
											public IFuture<Void> execute(IInternalAccess ia)
											{
												((JCCAgent)ia).getControlCenter().getPCC().setStatusText("Copy error: "+sel_1+" "+ex.getMessage());
												return IFuture.DONE;
											}
										});
//										jcc.setStatusText("Copy error: "+sel_1+" "+ex.getMessage());
