package agentkeeper.wegfindung;

import java.util.*;

import agentkeeper.map.InitMapProcess;
import agentkeeper.map.MoveAction;
...
public class ASternSuche {
	...
	private final wegpunkt _start;
	private ArrayList<wegpunkt> _offeneliste;
	private ArrayList<wegpunkt> _geschlosseneliste;
	private final Grid2D _space;
	private Vector2Int _zielvector;
	private wegpunkt _niedrigster;
	private boolean _direkt;
	Stack<Vector2Int> _pfad;

	public ASternSuche(IVector2 start, IVector2 ziel, Grid2D space, boolean direkt) {
		...
		_offeneliste = new ArrayList<wegpunkt>();
		
		//Liste der Wegpunkte die schon geprueft wurden
		_geschlosseneliste = new ArrayList<wegpunkt>();

		//Der Startwegpunkt
		_start = new wegpunkt(startvector, startvector, 0, 1000);
		...
		while (true) {
			...
			for (Vector2Int vector : richtungen) {
				...
				if (begehbar(vector) && !(istEnthalten(_geschlosseneliste, vector))) {
					...
					wegpunkt neueroffener = new wegpunkt(vector, _niedrigster.gibpunkt(), gwert, hwert);
	...
	private wegpunkt gibMinFkostenPunkt() {
		int minFKosten = Integer.MAX_VALUE;
		int fKosten;
		wegpunkt tmp, minFkostenPunkt = null;
	...
	private boolean istEnthalten(ArrayList<wegpunkt> wegpunkte, Vector2Int vector) {
		for (wegpunkt weg : wegpunkte) {
	...
	private int gibIndexOfWegpunkt(ArrayList<wegpunkt> wegpunkte, Vector2Int vector) {
		int i = 0;
		for (wegpunkt weg : wegpunkte) {
			if (weg.gibpunkt().equals(vector)) {
				return i;
	...
	//Wegkosten)
	private int berechneGwert(Vector2Int punkt, wegpunkt vorpunkt) {
