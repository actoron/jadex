...
### Modifying the existing plan to support the request format and introduce a new plan for adding word pairs

-   Create a new EnglishGermanAddWordPlanC1 as passive plan, that handles add-new-wordpair requests. In its body method, the plan should check whether the format is correct (using a `java.util.StringTokenizer`). 
If it is ok, it should retrieve the hashtable containing the word pairs via: `Map words = (Map)getBeliefbase().getBelief("egwords").getFact();` Assuming that the belief for storing the wordpairs is named "egwords". 
Now the plan has to check if the English word is already contained in the map (using `words.containsKey(eword)`) and if it is not contained, it should be added (using `words.put(eword, gword)`).
...
### Updating the ADF to incorporate the new plan and the new belief* 
...
##Exercise C2 - Beliefsets

Using a belief set for storing the word-pairs and employing beliefbase queries to look-up a word in the word table belief set. In this example each word pair is saved in a data structure called `jadex.commons.Tuple` which is a list of entities similar to an object array. 
In contrast to an object array two tuples are considered to be equal when they contain the same objects.
*Of course, in belief sets arbitrary Java objects can be stored, not just Tuples.*
