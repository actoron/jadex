...
-   We create a new *TranslationBDI* agent Java file and add the ```@Agent``` annotation to the class itself. Furthermore we need two fields, one for the agent API called *agent* and another one for the *wordtable* of type Map&lt;String, String&gt;. 
As you will remember, we need to add the ```@Agent``` annotation to the agent field and we make the wordtable a belief of the agent by adding ```@Belief```.
...
We name the goal class *Translate* and give it two String fields, one called *eword* and one called *gword* for the English and German word respectively.  
...
A goal if different from a plan because it describes an objective without exactly stating how it should be achieved. 
This means that different plans can be tried out to finally reach a given objective. Besides the specification of plans that can in principle help achieving a goal, the means-end reasoning process can be adjusted in many ways use the various BDI flags.
...
Think about what happens if the second plan would also throw an exception? Also ask yourself what the agent would do when the plans would be declared in different order or if the second would have a higher priority.  
Verify your thoughs by trying these things out in the source code.
...
If this is the case and no more plans exist although the goal has not been fulfilled yet, the *recur* setting determines how is proceeded. 
...
For this purpose we use the method ```scheduleStep()``` which is available via the agent API.   
As parameters we first add a new component step which adds the new word pair to the dictionary using ```wordtable.put("bugger", "Flegel")```.  
The second parameter is the delay for the step, here we use *3000* to state that we want it to be executed in 3 seconds. 
...
As we have illustrated in the introduction of this lecture, different goal types are available in Jadex.  
To create a goal of a different type, it is sufficient to use the corresponding condition types, i.e. a *maintain condition* in case of a maintain goal.  
...
-   We delete the translation goal and instead create a new *MaintainStorageGoal* as inner class. 
...
We want the maintain condition to trigger when the number of entries in the dictionary exceeds a given number.  
As consequence a plan for removing entries is triggered and it will remove entries until less than 3 word pairs are contained.
...
-   As next step we add a method plan called *removeEntry* without parameters and return value.  
