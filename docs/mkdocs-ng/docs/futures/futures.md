...
You can also wrap a *void* return type like this: ```public IFuture<Void> setHelloString(String name)```.
The method can then create a Future with Void type: ```new Future<Void>()``` and return it.
