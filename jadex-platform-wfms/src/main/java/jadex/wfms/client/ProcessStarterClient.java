...
public class ProcessStarterClient extends JFrame implements IClient
{
	...
	public ProcessStarterClient(IClientService clntService)
	{
		...
		for (Iterator it = clientService.getProcessRepositoryService(this).getProcessModelNames(this).iterator(); it.hasNext(); )
...
//				adminService.addProcessModel(ProcessStarterClient.this, name, path);
				clientService.getProcessRepositoryService(ProcessStarterClient.this).addProcessModel(ProcessStarterClient.this, path);
				((DefaultListModel) processList.getModel()).clear();
				for (Iterator it = clientService.getProcessRepositoryService(ProcessStarterClient.this).getProcessModelNames(ProcessStarterClient.this).iterator(); it.hasNext(); )
					((DefaultListModel) processList.getModel()).addElement(it.next());
			}
		});
