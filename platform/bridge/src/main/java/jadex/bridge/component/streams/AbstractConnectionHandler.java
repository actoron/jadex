...
//	protected IMessageFeature mf;
	...
	public AbstractConnectionHandler(IInternalAccess component, Map<String, Object> nonfunc)
	{
		...
		this(component, nonfunc, 3, Starter.getScaledRemoteDefaultTimeout(component.getIdentifier(), 1.0/3), 
			Starter.getScaledRemoteDefaultTimeout(component.getIdentifier(), 1.0/2));
	}
	...
	public TimerTask waitForRealDelay(long delay, final IComponentStep<?> step)
	{
		if(timer==null)
		{
			synchronized(this)
			{
				if(timer==null)
				{
					timer = new java.util.Timer(component.getIdentifier().getName()+".message.timer", true);
				}
			}
		}
