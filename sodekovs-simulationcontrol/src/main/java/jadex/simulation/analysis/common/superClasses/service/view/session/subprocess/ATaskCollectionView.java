...
//						list.setEnabled(false);
						...
						listScroller.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED), "Ausf�hrungen"));
...
//									}
						...
						generalTaskProperties.getTextField("Activit�tsname").setText(activity.getName());
//						System.out.println(activity);
//						System.out.println(activity.getClazz());
						generalTaskProperties.getTextField("Activit�tsklasse").setText(activity.getClazz().getTypeName().toString());
...
//				}
	...
	public void addTask(final IATask task, final IATaskView view)
	{
		synchronized (mutex)
		{
			final String name = "Ausf�hrung " + task.getTaskNumber();
	...
	public void update(ATaskEvent event)
	{
		...
		if (event.getCommand().equals(AConstants.TASK_LAEUFT))
		{
			SwingUtilities.invokeLater(new Runnable()
			{
				public void run()
				{
					String name = "Ausf�hrung " + task.getTaskNumber();
