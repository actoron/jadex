		...
		while (true) {
			...
			if (((Integer) getBeliefbase().getBelief("memoryLoggerCounter").getFact()).intValue() == -1) {
				...
				try {
					...
					new PrintStream(fout).println(Runtime.getRuntime().totalMemory() / 1024000);
...
// HashMap beliefbaseFacts = (HashMap)
...
// long experimentsPerRowToMake = ((Long)
// beliefbaseFacts.get(Constants.EXPERIMENTS_PER_ROW_TO_DO)).longValue();
// int totalRuns = ((Integer)
// beliefbaseFacts.get(Constants.TOTAL_EXPERIMENT_COUNTER)).intValue();
// int expInRow = ((Integer)
// beliefbaseFacts.get(Constants.ROW_EXPERIMENT_COUNTER)).intValue();
// int rowCounter = ((Integer)
// beliefbaseFacts.get(Constants.EXPERIMENT_ROW_COUNTER)).intValue();
//
...
// HashMap<Long, ArrayList<ObservedEvent>> observedEvents = new
// HashMap<Long, ArrayList<ObservedEvent>>();
//
...
// ((HashMap)
// args.get(Constants.SIMULATION_FACTS_FOR_CLIENT)).put(Constants.EXPERIMENT_ID,
// experimentID);
//		
...
// System.out.println("#*****************************************************************.");
//			
// System.out.println("#StartSimulationExpPlan# Started new Simulation Experiment. Nr.:"
// + experimentID + "(" + totalRuns + ") with Optimization Values: "
// + simConf.getOptimization().getData().getName() + " = " +
// simConf.getOptimization().getParameterSweeping().getCurrentValue());
// totalRuns++;
// expInRow++;
//
...
// beliefbaseFacts.put(Constants.TOTAL_EXPERIMENT_COUNTER, new
// Integer(totalRuns));
// beliefbaseFacts.put(Constants.ROW_EXPERIMENT_COUNTER, new
// Integer(expInRow));
// getBeliefbase().getBelief("generalSimulationFacts").setFact(beliefbaseFacts);
//
// // update experiment counter in intermediate results
// IntermediateResult interRes = (IntermediateResult)
// getBeliefbase().getBelief("intermediateResults").getFact();
...
// rowCounter++;
// beliefbaseFacts.put(Constants.EXPERIMENT_ROW_COUNTER, new
// Integer(rowCounter));
// getBeliefbase().getBelief("generalSimulationFacts").setFact(beliefbaseFacts);
//
// // store results of row
// HashMap<Integer, ExperimentResult> experimentResults = (HashMap<Integer,
// ExperimentResult>)
// getBeliefbase().getBelief("experimentResults").getFact();// contains
// // the
// // results of the
// // experiments done
// // in this row
// HashMap rowResults = (HashMap)
// getBeliefbase().getBelief("rowResults").getFact();
//
...
// // }
// //
// ArrayList<ExperimentResult> experimentList = new
// ArrayList<ExperimentResult>(experimentResults.values());
...
// getBeliefbase().getBelief("intermediateResults").setFact(new
// IntermediateResult(rowCounter, 0, simConf));
...
// String type =
// "..\\jadex-applications-bdi\\target\\classes\\jadex\\bdi\\simulation\\client\\ClientSimulator.agent.xml";
...
// }
//
// private void startApplication(String appName, String fileName, String
// configName, Map args) {
//
// try {
...
// executionService.createComponent(appName, fileName, new
// CreationInfo(configName, args, null, false, false), null);
//
// } catch (Exception e) {
