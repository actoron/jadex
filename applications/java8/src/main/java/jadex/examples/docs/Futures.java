...
public class Futures {
    ...
    public IFuture<String> addAndToStringJava8(int a, int b) 
    {
        ...
        addService.add(a,b).then(sum ->
            toStringService.toString(sum).delegate(res))
        .catchErr(ex -> res.setException(ex));

        // todo???
        //addService.add(a,b).addResultListener(SResultListener.delegateExceptions(res, sum -> toStringService.toString(sum).delegate(res)));
        return res;
    }
...
//                stopWork();
    ...
    private void samples() 
    {
        ...
        fut.then(res -> threeDots(res)).catchErr(ex -> myFut.setException(ex));

        // delegate results, use exceptions:
        fut.then(res -> myFut.setResult(res)).catchErr(ex -> threeDots(ex));
