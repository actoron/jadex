	...
	public class SendMediumMessageStep implements IComponentStep {
		...
		public Object execute(IInternalAccess ia) {
			if (message instanceof HelpRequest) {
				...
				ServiceHelper.handleConsoleMsg(getServiceProvider(), new ConsoleMessage(ConsoleMessage.TYPE_ADAPTIVITY, msg), getLogger());
		...
		if (reply.getReceiverIds().contains(id)) {
			...
			if (reply.getDefectAgentId().equals(id)) {
				...
				ServiceHelper.handleConsoleMsg(getServiceProvider(),
						new ConsoleMessage(ConsoleMessage.TYPE_ADAPTIVITY, id + " removes " + reply.getTakenRoles().size() + " deficient roles " + reply.getTakenRoles()), getLogger());
				ServiceHelper.handleConsoleMsg(getServiceProvider(),
						new ConsoleMessage(ConsoleMessage.TYPE_ADAPTIVITY, id + " adds " + reply.getVacantRoles().size() + " new roles " + reply.getVacantRoles()), getLogger());
				...
				if (vacantInput || vacantOutput || takenInput || takenOutput) {
					for (Role role : assignedRoles) {
						...
						if (vacantOutput && reply.getReplaceAgentId().equals(role.getPrecondition().getTargetAgent())) {
							...
							ServiceHelper.handleConsoleMsg(getServiceProvider(), new ConsoleMessage(ConsoleMessage.TYPE_ADAPTIVITY, id + " changes input from "
									+ role.getPrecondition().getTargetAgent() + " to " + newInput), getLogger());
						...
						if (vacantInput && reply.getReplaceAgentId().equals(role.getPostcondition().getTargetAgent())) {
							...
							ServiceHelper.handleConsoleMsg(getServiceProvider(), new ConsoleMessage(ConsoleMessage.TYPE_ADAPTIVITY, id + " changes output from "
									+ role.getPostcondition().getTargetAgent() + " to " + newOutput), getLogger());
