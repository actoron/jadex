...
import sodekovs.investigation.model.Configuration;
import sodekovs.investigation.model.Data;
import sodekovs.investigation.model.Optimization;
import sodekovs.investigation.model.ParameterSweeping;
...
public class StartSimulationExperimentsPlan extends Plan {
	...
	public void body() {
		...
		System.out.println("#StartSimulationExpPlan# Path: " + investigationConf.getApplicationReference());
		...
		clientArgs.put(Constants.APPLICATION_FILE_AS_XML_STRING, FileHandler.readFileAsString(investigationConf.getApplicationReference()));
		// read the *.configuration.xml file from the file system
		clientArgs.put(GlobalConstants.CONFIGURATION_FILE_AS_XML_STRING, FileHandler.readFileAsString((String) getBeliefbase().getBelief("investigationDescriptionFile").getFact()));
		...
		if (investigationConf.getOptimization().getParameterSweeping() != null) {
			if (investigationConf.getOptimization().getParameterSweeping().getStrategy().equals("linear")) {
				sweepParameters(investigationConf, applicationArgs);
			} else if (investigationConf.getOptimization().getParameterSweeping().getStrategy().equals("recursively")){
				recusivelySweepParameter(investigationConf, applicationArgs);
			} else if (investigationConf.getOptimization().getParameterSweeping().getStrategy().equals("no sweeping to be performed.")){
				//Do nothing
			}
		} else {
			investigationConf.getOptimization().setParameterSweeping(new ParameterSweeping());
			investigationConf.getOptimization().getParameterSweeping().setCurrentConfiguration("Simulation without Parameter Sweeping");
			investigationConf.getOptimization().getParameterSweeping().setStrategy("no sweeping to be performed.");
		}
		...
		clientArgs.put(GlobalConstants.CURRENT_PARAMETER_CONFIGURATION, investigationConf.getOptimization().getParameterSweeping().getCurrentConfiguration());
		...
		rowResult.setOptimizationConfiguration(investigationConf.getOptimization().getParameterSweeping().getCurrentConfiguration());
...
//	}
	 ...
	 * @param args
	 */
	private void sweepParameters(InvestigationConfiguration investigationConf, Map args) {
		String currentParameterValues = "";
		Optimization opt = investigationConf.getOptimization();
	...
	private void recusivelySweepParameter(InvestigationConfiguration investigationConf, Map args) {
