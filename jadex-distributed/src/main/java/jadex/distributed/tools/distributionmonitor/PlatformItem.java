...
public class PlatformItem extends JPanel {
	...
	private int port;
	/*
	 * CPU and RAM usage can vary and need to be updated regulary; usage varies
	 * from 0 to 100, from 0 to 100%
	 */
	private byte cpuUsage = 15; // dummy data; gather it from JMX
	private byte ramUsage = 70;

	public PlatformItem(String ip, int port) {
		this.ip = ip;
		this.port = port;
	...
	protected void paintComponent(Graphics gi) {
		...
		// TODO build here string with stringbuilder, 100% ' '+45% to better
		// align strings
		g.drawString("dummy", 40, 32); // TODO string look ugly with
		// drawString(...); beautify with
		...
		g.setPaint(new Color(0, 0, 0));
		// TODO build here string with stringbuilder; ( == ) ? : if-else
		// shortcut may good choice here
		g.drawString("dummy", 40, 52);

		// g.setPaint(new Color(0, 255, 0)); // R, G, B; red, green, blue
		// g.fillRect(3, 20, 100, 15); // 100 auf aktuelle CPU-Auslastung
		// anpassen
	}
