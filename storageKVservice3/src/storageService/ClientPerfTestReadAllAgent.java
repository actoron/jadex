...
public class ClientPerfTestReadAllAgent {
	...
	/**
	 * test readAll performance.
	 * @param storageClientService
	 */
	private void readAll(IStorageClientService storageClientService) {
		final long start = System.currentTimeMillis();
		Timestamp ts = new Timestamp(start);
		System.out.println("Start readAll: " + ts.toString());
			...
			public void resultAvailable(List<VersionValuePair> result) {
				if (result.size() == 0) {
					...
					while (it.hasNext()) {
						...
						System.out.println(key + "=" + vvp.toString());
						System.out.println("ReadAll test finished: "
								+ ts2.toString());
						System.out.println("ReadAll test with " + numberOfTests
