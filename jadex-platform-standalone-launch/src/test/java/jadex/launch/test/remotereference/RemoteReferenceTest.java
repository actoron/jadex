...
public class RemoteReferenceTest extends TestCase
{
	public void	testRemoteReference()
	{
		...
		IExternalAccess	platform1	= Starter.createPlatform(new String[]{"-platformname", "testcases_*",
			...
			"-component", "jadex/launch/test/remotereference/LocalServiceProviderAgent.class"}).get();
		
		// Find local service (as local provided service proxy).
		ILocalService	service1	= SServiceProvider
			.getService(platform1.getServiceProvider(), ILocalService.class).get();
		...
		IExternalAccess	platform2	= Starter.createPlatform(new String[]{"-platformname", "testcases_*",
			"-saveonexit", "false", "-welcome", "false", "-autoshutdown", "false", "-gui", "false", "-awareness", "false", "-printpass", "false",
			"-component", "jadex/launch/test/remotereference/SearchServiceProviderAgent.class"}).get();
		...
		IComponentManagementService	cms1	= SServiceProvider
			.getServiceUpwards(platform1.getServiceProvider(), IComponentManagementService.class).get();
		cms1.createComponent(null, "jadex/platform/service/remote/ProxyAgent.class", new CreationInfo(args1), null).get();
		Map<String, Object>	args2	= new HashMap<String, Object>();
		args2.put("component", platform1.getComponentIdentifier());
		IComponentManagementService	cms2	= SServiceProvider
			.getServiceUpwards(platform2.getServiceProvider(), IComponentManagementService.class).get();
		cms2.createComponent(null, "jadex/platform/service/remote/ProxyAgent.class", new CreationInfo(args2), null).get();
		
		// Search for remote search service from local platform
		ISearchService	search	= SServiceProvider
			.getService(platform1.getServiceProvider(), ISearchService.class, RequiredServiceInfo.SCOPE_GLOBAL).get();
		// Invoke service to obtain reference to local service.
		ILocalService	service2	= search.searchService("dummy").get();
