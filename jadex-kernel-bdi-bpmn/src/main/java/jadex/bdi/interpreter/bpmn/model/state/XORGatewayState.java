...
//    	this.successorId = null;
    	...
    	// check conditions and add successor
    	Iterator edges = getOutgoingEdges().iterator();
    	IBpmnState successor = null;
    	while (successor == null && edges.hasNext())
		{
    		IBpmnTransition edge = (IBpmnTransition) edges.next();
			if (edge instanceof ConditionalFlow)
			{
	    		Boolean b = context.evalJadexOQLCondition(((ConditionalFlow)edge).getCondition());
				if (b.booleanValue())
				{
					successor = (IBpmnState) edge.getTargetState();
				}
			}
		}
...
//    	setFinished(true);
    	if (successor != null)
    		context.activateState(successor);
