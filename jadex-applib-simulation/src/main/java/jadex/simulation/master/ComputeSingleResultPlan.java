...
public class ComputeSingleResultPlan extends Plan {
	...
	/**
	 * Create buckets, in order to filter events, that belong to observers which only need the last "observedValue".
	 * 
	 * @param simConf
	 */
	...
	/**
	 * Filter those events that are of interest. For some observer data (like duration) only the last observed event is of interest, whereas others like a Logger need every observed value. Right now: Add last value for a observer to results list.
	 * 
	 * @param events
	...
	/**
	 * Returns the list of observed events ascendingly ordered by relative timestamp.
	 */
