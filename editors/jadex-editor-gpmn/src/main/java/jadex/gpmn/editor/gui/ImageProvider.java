...
public class ImageProvider
{
	...
	private Image generateSymbol(String name, Dimension size)
	{
		...
		else if ("supp_edge".equals(name))
		{
			ret = new BufferedImage(size.width, size.height, BufferedImage.TYPE_4BYTE_ABGR_PRE);
			Graphics2D g = ((BufferedImage) ret).createGraphics();
			double offsetx = size.width * 0.15;
			double offsety = size.height * 0.45;
			double ellw = size.width * 0.25;
			double ellh = size.height * 0.25;
			Rectangle2D rect = new Rectangle2D.Double(offsetx, offsety,
					size.width - offsetx - offsetx - ellw * 0.5, size.height - offsety - offsety);
			Ellipse2D ell = new Ellipse2D.Double(size.width - offsetx - ellw, size.height * 0.5 - ellh * 0.5, ellw, ellh);
