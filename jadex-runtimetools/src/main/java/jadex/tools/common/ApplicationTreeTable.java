 ...
 */
public class ApplicationTreeTable extends JScrollPane
{
	//-------- constants --------
	...
	public static String NODE_APPLICATION = "application_node";
	...
	public static UIDefaults icons = new UIDefaults(new Object[]{
		// Node icons.
		NODE_APPLICATION, SGUI.makeIcon(AgentTreeTable.class, "/jadex/tools/common/images/application.png"), 
	 ...
	 *  Open the gui.
	 */
	public ApplicationTreeTable(String platname)
	{
		...
		addNodeType(new TreeTableNodeType(NODE_APPLICATION, new Icon[]{icons.getIcon(NODE_APPLICATION)}, new String[]{"name"}, new String[]{"Name"}));
		...
		this.treetable = new JTreeTable(new DefaultTreeTableModel(platform, getNodeType(NODE_APPLICATION).getColumnNames()));
...
//		header.setColumnWidths(treetable.getColumnModel().getColumn(0), 100, 100, -1);
	 ...
	 *  Add an agent.
	 */
	public void addApplication(IApplicationContext appcontext)
	{		
		Map values = new HashMap();
		values.put("name", appcontext.getName());

		platform.add(new DefaultTreeTableNode(getNodeType(NODE_APPLICATION), appcontext, values));
...
//		System.out.println("Starter set props: "+props);
	 ...
	 */
	public void updateApplication(IApplicationContext appcontext)
	{
		Map values = new HashMap();
		values.put("name", appcontext.getName());

		DefaultTreeTableNode node = platform.getChild(appcontext);
		node.setValues(values);
		node.setUserObject(appcontext);
	 ...
	 *  @param description
	 */
	public void removeApplication(IApplicationContext appcontext)
	{
	 ...
	 */
	public DefaultTreeTableNode[] getAllApplications()
	{
