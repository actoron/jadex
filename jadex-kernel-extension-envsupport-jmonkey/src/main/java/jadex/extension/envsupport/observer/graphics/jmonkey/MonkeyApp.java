...
public class MonkeyApp extends SimpleApplication implements AnimEventListener {
	...
	private boolean _chaseCamera = false;
	...
	private boolean _hudactive = false;

	private boolean _isGrid;

	private Screen _hudscreen;
	...
	private void initRenderer() {
		...
		pssmRenderer.setDirection(new Vector3f(-.5f, -.5f, -.5f)
	...
	private void initRoot() {
		...
		_gridHandler = new monkeyApp_Grid(_appDimension, _spaceSize,
				assetManager, _isGrid);
	...
	public void initCam() {
		...
		// The Focus Camera
		_focusCam = new FocusCamera(cam, _staticNode, inputManager);
		_focusCam.setUpVector(Vector3f.UNIT_Y);
		_focusCam.setSmoothMotion(true);
		_focusCam.setDragToRotate(true);
		_focusCam.setDefaultDistance(1000f);
		_focusCam.setMaxDistance(2000f);
		_focusCam.setMinDistance(50f);
		_focusCam.setZoomSpeed(20f);
		_focusCam.setEnabled(false);
	...
	public void setStaticGeometry(Node staticNode) {
		...
		if (terra != null) {
			...
			_terrain.setLocalTranslation(_appDimension / 2, 0,
					_appDimension / 2);
	...
	public void setHeight() {
		if (_terrain != null) {
			...
			_terrain = new TerrainQuad("Terrain", 65, 513,
	...
	private void fireSelection() {
		...
		if(_chaseCamera)
		{
			...
			_focusCam.setSpatial(selectedspatial);
			else
			_focusCam.setSpatial(_staticNode);
		}
	...
	/**
	 * @param _selectedSpatial
	 *            the _selectedSpatial to set
	 */
	...
	/**
	 * @param _selectedTarget
	 *            the _selectedTarget to set
	 */
