...
@ProvidedServices(@ProvidedService(type = IDiscoveryService.class, implementation = @Implementation(expression = "$component")))
@RequiredServices({
...
public class MulticastDiscoveryAgent extends MicroAgent implements
		IDiscoveryService {
	// -------- attributes --------
	...
	/**
	 * Flag indicating that the agent is started and the send behavior may be
	 * activated.
	 */
	...
	public IFuture agentCreated() {
		...
		this.reader = JavaReader.getReader(new XMLReporter() {
			public void report(String message, String type, Object related,
	...
	protected void initArguments() {
		try {
			this.address = InetAddress
					.getByName((String) getArgument("address"));
		} catch (UnknownHostException e) {
			throw new RuntimeException(e);
		}
		if (address == null)
			throw new NullPointerException("Cannot get address: "
					+ getArgument("address"));
	...
	public void executeBody() {
		...
		// Wait before starting send behavior to not miss fast awareness
		// pingpong replies,
		// because receiver thread is not yet running. (hack???)
		startReceiving().addResultListener(
	...
	public void send(final AwarenessInfo info) {
		try {
			byte[] data = JavaWriter.objectToByteArray(info, getModel()
					.getClassLoader());
	...
	/**
	 * Start sending awareness infos. (Ends automatically when a new send
				...
				public Object execute(IInternalAccess ia) {
					if (!killed && sendid.equals(getSendId())) {
						...
						send(new AwarenessInfo(root,
								AwarenessInfo.STATE_ONLINE, delay, includes,
								excludes));
	...
	public IFuture startReceiving() {
		...
		getServiceContainer().getRequiredService("threadpool")
				.addResultListener(createResultListener(new IResultListener() {
					public void resultAvailable(Object result) {
						...
						tp.execute(new Runnable() {
							public void run() {
								// todo: max ip datagram length (is there a
								// better way to determine length?)
									...
									try {
										...
										synchronized (MulticastDiscoveryAgent.this) {
											if (!killed) {
												if (receivesocket != null
														&& (receivesocket
																.getPort() != curport || !SUtil
																.equals(curaddress,
																		myaddress))) {
													receivesocket
															.leaveGroup(myaddress);
													receivesocket.close();
													receivesocket = null;
												}
												if (receivesocket == null) {
													try {
														receivesocket = new MulticastSocket(
																curport);
														receivesocket
																.joinGroup(curaddress);
														myaddress = curaddress;
													} catch (Exception e) {
														receivesocket = null;
														getLogger().warning(
																"Awareness error when joining mutlicast group: "
																		+ e);
										...
										byte[] target = new byte[pack
												.getLength()];
										System.arraycopy(buf, 0, target, 0,
												pack.getLength());

										final AwarenessInfo info = (AwarenessInfo) Reader
												.objectFromByteArray(
														reader,
														target,
														getModel()
													...
													.addResultListener(
															new DefaultResultListener() {
																public void resultAvailable(
																	...
																	// send(new
																	// AwarenessInfo(root,
																	// AwarenessInfo.STATE_ONLINE,
																	// delay,
																	// includes,
																	// excludes));
																	// waitFor((long)(Math.random()*500*cnt),
																	// this);
																	// }
																	// return
																	// null;
					...
					public void exceptionOccurred(Exception exception) {
						getLogger().warning(
								"Awareness agent problem, could not get threadpool service: "
										+ exception);
