...
import sodekovs.marsworld.microcarry.MicroCarryAgent;

/**
 *  Move an object towards a destination.
 */
public class MicroMoveTask extends AbstractTask
{
	//-------- constants --------
	
	/** The destination property. */
	public static final String	PROPERTY_TYPENAME = "micromove";
	...
	public void execute(IEnvironmentSpace space, ISpaceObject obj, long progress, IClockService clock)
	{
		...
		final IMicroExternalAccess agent = (IMicroExternalAccess)getProperty(PROPERTY_SCOPE);
//		final IBDIExternalAccess agent = (IBDIExternalAccess)getProperty(PROPERTY_SCOPE);
		...
		if(objects!=null)
		{
			agent.scheduleStep(new IComponentStep<Void>()
			{
				...
				{		
					MicroCarryAgent mcia = (MicroCarryAgent)ia;
//					IBDIInternalAccess bia = (IBDIInternalAccess)ia;
					for(Iterator it=objects.iterator(); it.hasNext(); )
					{
						...
						{
							if(!mcia.getObjects().contains(so)){
								mcia.addObject(so);
							}
