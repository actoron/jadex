...
public class JCCAgent implements IComponentStep<Void> {
	...
	public IFuture<Void> execute(final IInternalAccess agent) {
		...
		if (platforms == null) {
			...
			SJCC.getRootAccess(agent.getExternalAccess())
					.addResultListener(
							new SwingExceptionDelegationResultListener<IExternalAccess, Void>(
									ret) {
			...
			agent.getLogger().info(
					"Searching for platforms matching '" + platforms + "'.");

			agent.getServiceContainer()
					.searchServices(IComponentManagementService.class,
							RequiredServiceInfo.SCOPE_GLOBAL)
					.addResultListener(
							new IntermediateExceptionDelegationResultListener<IComponentManagementService, Void>(
									ret) {
								boolean found = false;

								public void intermediateResultAvailable(
										IComponentManagementService cms) {
									IComponentIdentifier cid = ((IService) cms)
								...
								public void finished() {
									// If no platform found, search again after
									// 1 second.
									if (!found) {
	...
	public IFuture<Void> agentKilled(IInternalAccess agent) {
		...
		cc.shutdown().addResultListener(
				agent.createResultListener(new DelegationResultListener<Void>(
						ret)));
	 ...
	 */
	protected IFuture<Void> initControlCenter(IInternalAccess agent,
			IExternalAccess platform) {
		Future<Void> ret = new Future<Void>();
		if (this.cc == null) {
			this.cc = new ControlCenter();
			cc.init(agent.getExternalAccess(),
					platform,
					new String[] { StarterPlugin.class.getName(),
							...
							CoordinationPlugin.class.getName() }, saveonexit)
					.addResultListener(
							agent.createResultListener(new DelegationResultListener<Void>(
									ret)));
		} else {
