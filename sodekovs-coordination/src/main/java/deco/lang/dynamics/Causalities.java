 ...
 * Direct and decentralized links are distinguished. The detailed configurations of links can be specified as well.
 * 
 * @author Jan Sudeikat
 * 
 */
...
public class Causalities {
	...
	/**
	 * Returns the {@link DecentralMechanismLink} where {@link DecentralMechanismLink#getRealization()} matches the given realization name.
	 * 
	 * @param nam
	 *            the given realization name
	 * @return the {@link DecentralMechanismLink} where {@link DecentralMechanismLink#getRealization()} matches the given realization name or <code>null</code> if non of the realization names matches.
	 */
	public DecentralMechanismLink getDmlByRealizationName(String name) {
		for (DecentralMechanismLink dml : this.dml) {
			if (dml.getRealization().equals(name)) {
				return dml;
			}
		}
	...
	public DecentralizedCausality getRealizationByName(String name) {
		for (DecentralizedCausality causality : realizations) {
			if (causality.getId().equals(name)) {
				return causality;
			}
		}
	...
	public void setDirectLinkRealizations(ArrayList<DirectCausality> directLinkRealizations) {
	...
	public DirectCausality getDirectLinkRealizationByName(String name) {
		for (DirectCausality c : this.directLinkRealizations) {
			if (c.getId().equalsIgnoreCase(name))
				return c;
		}
		return null;
	}
	...
	public MechanismConfiguration getDCMRealizationByImplName(String name) {
		for (DecentralizedCausality c : this.realizations) {
			if (c.getMechanismConfiguration().getMechanism_id().equalsIgnoreCase(name))
				return c.getMechanismConfiguration();
		}
		return null;
	}
