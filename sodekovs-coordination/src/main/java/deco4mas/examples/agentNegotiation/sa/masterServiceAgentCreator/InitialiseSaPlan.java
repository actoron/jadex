...
public class InitialiseSaPlan extends Plan {
	...
	public void body() {
		try {
			...
			ServiceAgentConfiguration[] configs = (ServiceAgentConfiguration[]) getBeliefbase().getBeliefSet("serviceAgentConfigurations").getFacts();
			...
			for (ServiceAgentConfiguration serviceAgentConfig : configs) {
				...
				saLogger.info(serviceType.getName() + " with charakter " + agentType.getTypeName() + " C(" + agentType.getCostCharacter() + "), D(" + agentType.getDurationCharacter() + "), B("
						+ agentType.getBlackoutCharacter() + ")");
				Map args = new HashMap();
				args.put("providedService", serviceType);
				args.put("serviceAgentType", agentType);

				cms.createComponent(saName, "deco4mas/examples/AgentNegotiation/sa/ServiceAgent.agent.xml", new CreationInfo(null, args, interpreter.getParent().getComponentIdentifier()), null);
				id++;
