...
public class BDICoordinationInformationInterpreter extends SimplePropertyObject implements ICoordinationInformationInterpreter {
	 ...
	 * (non-Javadoc)
	 * 
	 * @seedeco4mas.coordinate.interpreter.coordination_information. ICoordinationInformationInterpreter#processPercept(jadex.bridge.ISpace, java.lang.String, java.lang.Object, jadex.bridge.IAgentIdentifier,
	 * jadex.adapter.base.envsupport.environment.ISpaceObject)
	 */
	// @Override	
...
//		final String thisAgentType = ((IApplicationContext) ((AbstractEnvironmentSpace) space).getContext()).getAgentType(agent);
		final String thisAgentType = ((AbstractEnvironmentSpace) space).getContext().getComponentType(agent);
		

		if (invoke) {
//			IAMS ams = (IAMS) ((IApplicationContext) space.getContext()).getPlatform().getService(IComponentManagementService.class);
			IComponentManagementService ams = (IComponentManagementService) ((AbstractEnvironmentSpace) space).getContext().getServiceContainer().getService(IComponentManagementService.class);
							...
							try {
								for (int i = 0; i < metainfos.length; i++) {
									...
									if (ADD.equals(metainfos[i][0])) {
										...
										// Map<String, Object[]> applicablePerceiveRoles = (Map<String, Object[]>) decom4MasMap.get(Constants.ROLE_DEFINITIONS_FOR_PERCEIVE);
										...
										// // coordInfo.addValue(Constants.TO_AGENTS, toAgents);
										...
										// Check whether there is a perceive definition for this agentType
										if (applicableAgentTypes.get(getAgentType(exta)) != null) {
											...
											// Check whether this percept should be processed by this agent, i.e. if it is applicable. Does this agent have the received DCM as a perceive-event?
											if (applicablePerceiveRoles.get(dcmName) != null) {
												...
												if (CheckRole.checkForPerceive(dci.getRef(), exta)) {
													...
													HashMap<String, Object> receivedParamDataMappings = (HashMap<String, Object>) coordinationSpaceObj.getProperty(Constants.PARAMETER_DATA_MAPPING);

													// TODO: ParameterMappings for PERCEIVE

													// DecentralCoordinationInformation decentralCoordInfo = ((DecentralCoordinationInformation) decom4MasMap
													// .get(Constants.DECENTRAL_COORDINATION_INFO_MAPPING));
													//
													if (elementType.equals(AgentElementType.BDI_BELIEFSET.toString())) {
														// System.out.println("BDICoordInfInterpreter# RECEIVED BELIEF_SET TO MANIPULATE:  " + elementId + " for " + exta.getAgentName());

														// check direction (POSITIVE -> add fact to beliefset / NEGATIVE -> remove fact from beliefset)
														if (dci.getCoordinationType().equals(CoordinationType.NEGATIVE)) {
															...
															belset.removeFact(facts[facts.length - 1]); // remove the last fact
															System.out.println("BDICoordInfInterpreter# Removed last fact from BELIEF_SET: " + elementId + " for " + exta.getAgentName());
														} else {
															// TODO: Distinguish between 'UPDATE Values' and 'ADD Values'
															// System.out.println("#tmp# percept: " + percept);
															// IBeliefSet belset = exta.getBeliefbase().getBeliefSet(metainfos[i][1]);
															IBeliefSet belset = exta.getBeliefbase().getBeliefSet(elementId);
															// belset.addFact(percept);
															belset.addFact(coordinationSpaceObj.getProperty(Constants.VALUE).toString());
															System.out.println("BDICoordInfInterpreter# Added " + coordinationSpaceObj.getProperty(Constants.VALUE).toString() + " to BELIEF_SET: "
																	+ elementId + " for " + exta.getAgentName());
														}
													} else if (elementType.equals(AgentElementType.BDI_BELIEF.toString())) {
														// System.out.println("BDICoordInfInterpreter# RECEIVED BELIEF TO MANIPULATE:  " + nameOfElement + " for " + exta.getAgentName());

														// check direction (POSITIVE -> add fact to belief / NEGATIVE -> remove fact from belief):
														if (dci.getCoordinationType().equals(CoordinationType.NEGATIVE)) {
															// IBelief bel = exta.getBeliefbase().getBelief(metainfos[i][1]);
															IBelief bel = exta.getBeliefbase().getBelief(elementId);
															bel.setFact(new Object()); // TODO better default value...
														...
														// System.out.println("BDICoordInfInterpreter# RECEIVED GOAL TO MANIPULATE:  " + nameOfElement + " for " + exta.getAgentName());

														// check direction (POSITIVE -> dispatch goal / NEGATIVE -> remove goal
														if (dci.getCoordinationType().equals(CoordinationType.NEGATIVE)) {
															IGoal[] goals = exta.getGoalbase().getGoals(elementId);
															goals[goals.length - 1].drop(); // TODO: heuristic for selection
															System.out.println("BDICoordInfInterpreter# Removed GOAL from GOALBASE : " + elementId + " for " + exta.getAgentName());
														} else {
															IGoal g = exta.getGoalbase().createGoal(elementId);
															// TODO: Implement the add parameters:
															// add parameters:
															...
															System.out.println("BDICoordInfInterpreter# Dispatched new GOAL: " + elementId + " for " + exta.getAgentName());
														}

													} else if (elementType.equals(AgentElementType.BDI_PLAN.toString())) {
														System.out.println("BDICoordInfInterpreter# Error!!! RECEIVED PLAN TO MANIPULATE:  " + elementId + " for " + exta.getAgentName()
																+ " Currently unable to process IPlan due limits of used jadex-version");

														// check direction (POSITIVE -> dispatch goal / NEGATIVE -> remove goal
														// // if (decentralCoordInfo.getCoordinationType().equals(CoordinationType.NEGATIVE)) {
														// // IGoal[] goals = exta.getGoalbase().getGoals(metainfos[i][1]);
														// // goals[goals.length - 1].drop(); // TODO: heuristic for selection
														// // System.out.println("BDICoordInfInterpreter# Removed GOAL from GOALBASE : " + nameOfElement + " for " + exta.getAgentName());
														// // } else {
														// // IGoal g = exta.getGoalbase().createGoal(nameOfElement);
														// // // TODO: Implement the parameters add parameters:
														// // // for (ParameterMapping pm :
														// // // ca.getParameter_mappings()){
														// // // g.getParameter(pm.getLocalName()).setValue(item.getValueByName(pm.getRef()));
														// // // }
														// // exta.getGoalbase().dispatchTopLevelGoal(g);
														// // System.out.println("BDICoordInfInterpreter# Dispatched new GOAL: " + nameOfElement + " for " + exta.getAgentName());
														// // }
														// }
													} else if (elementType.equals(AgentElementType.INTERNAL_EVENT.toString())) {
														// System.out.println("BDICoordInfInterpreter# RECEIVED GOAL TO MANIPULATE:  " + nameOfElement + " for " + exta.getAgentName());

														// check direction (POSITIVE -> event addition / NEGATIVE -> element removal):
														if (dci.getCoordinationType().equals(CoordinationType.NEGATIVE)) {
															...
															System.out.println("BDICoordInfInterpreter# Dispatched new InternalEvent: " + elementId + " for " + exta.getAgentName());
														}
...
//									else if (COORDINATE_INIT_PARTICIPANTS.equals(metainfos[i][0])) {
//										ISpaceObject coordinationSpaceObj = (ISpaceObject) percept;
//
//										if (coordinationSpaceObj.getProperty(CoordinationSpaceObject.AGENT_ELEMENT_TYPE).equals(AgentElementType.INTERNAL_EVENT)) {
//											// System.out.println("BDICoordInfInterpreter-" + thisAgentType + "# RECEIVED INTERNAL_EVENT TO MANIPULATE with metainfo: " + metainfos[i][1]);
//											IInternalEvent ie = exta.getEventbase().createInternalEvent(metainfos[i][1]);
//											HashMap values = (HashMap) ((ISpaceObject) percept).getProperty("VALUES");
//											ie.getParameter("deco4mas_info").setValue(values);
//											exta.getEventbase().dispatchInternalEvent(ie);
//											// System.out.println("BDICoordInfInterpreter# Dispatched Internal_Event " + ie.toString() + " for " + exta.getAgentName());
//										}
//									}
								}
							} catch (Exception e) {
...
//		return exta.getApplicationContext().getAgentType(exta.getAgentIdentifier());		
	}
